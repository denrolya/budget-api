...
class BankWebhooksRefreshCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        assert(\is_array($providerFilter));
        $includeInactive = (bool) $input->getOption('include-inactive');
        $dryRun = (bool) $input->getOption('dry-run');

        $allowedProviders = [];
        foreach ($providerFilter as $rawProvider) {
            assert(\is_string($rawProvider));
            try {
                $allowedProviders[] = BankProvider::from($rawProvider);
            } catch (ValueError) {
                $io->error(\sprintf(
        ...
        if (null !== $rawIntegrationId) {
            ...
            if (null === $integration) {
                $io->error(\sprintf('BankIntegration #%d not found.', $integrationId));
    ...
    private function buildCredentialFingerprint(BankIntegration $integration): string
    {
        ...
        return hash('sha256', $provider . '|' . $encoded);
    }
}
