...
final class ExpenseFeatureTest extends BaseApiTestCase
{
    ...
    /**
     * Creating an expense decrements the account balance by the given amount,
    ...
    /**
     * On creation the expense's converted values are calculated and stored
    ...
    /**
     * isDraft is a display/planning flag only. A draft expense is still fully
     * persisted and its amount is deducted from the account balance immediately,
     * exactly as a non-draft expense would be.
     */
    ...
    /**
     * The API must reject expenses with an amount of zero with HTTP 422.
     * Zero-amount expenses carry no financial meaning and should not be stored.
     */
    public function testCreateExpenseWithZeroAmountReturns422(): void
    {
    ...
    /**
     * The API must reject expenses with a negative amount with HTTP 422.
     * Negative amounts are not meaningful for expenses; refunds are modelled
     * via compensations or separate income transactions.
     */
    public function testCreateExpenseWithNegativeAmountReturns422(): void
    {
        $this->client->request('POST', self::EXPENSE_URL, [
            'json' => [
                'amount' => '-50',
     ...
     * and adjusts the account balance by the delta (old minus new amount).
    ...
    /**
     * Moving an expense to a different account reverses the balance effect on
    ...
    /**
     * Toggling isDraft on an existing expense does NOT trigger a recalculation
     * of converted values and does NOT adjust the account balance. isDraft is
     * not a "valuable" field — it is a display flag that carries no financial
     * weight.
     */
    public function testToggleIsDraftDoesNotChangeConvertedValuesOrBalance(): void
    {
    ...
    /**
     * Changing the category of an expense updates the transaction count on both
     * the old and the new category. No balance recalculation is triggered
     * because category is not a financially "valuable" field.
     */
    public function testUpdateExpenseCategoryUpdatesCategoryCount(): void
    {
        $this->mockAssetsManager->expects(self::exactly(1))->method('convert');
        ...
        self::assertEquals($eatingOutCountBefore + 1, $eatingOutCategory->getTransactionsCount(false));
    }

    // ── Delete ───────────────────────────────────────────────────────────────

    /**
     * Deleting an expense reverses its balance effect on the account (restores
     * the deducted amount) and removes it from both the account's and the
